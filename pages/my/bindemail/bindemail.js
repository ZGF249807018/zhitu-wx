...
Page({
  ...
  getpassword() {
    ...
    wx.request({
      url: webUrl + `/api/tenant/v1/tenant/sendEmail2`,
      method: 'POST',
      header: {
      ...
      data: {
        email: vm.data.inputcontentemail,
      },
  ...
  submit() {
    ...
    if (!vm.data.inputcontentpassword) {
      wx.showToast({
        title: '请输入验证码!',
        icon: 'none',
        duration: 1000,
    ...
    wx.request({
      ...
      success: function(res) {
        if (res.data.code === 200) {
          ...
          // wx.switchTab({
          //   url: 'pages/my/my?emial=' + inputcontentemail,
          // })
          wx.request({
            url: webUrl + '/api/tenant/v1/tenant/detail?tenantId=' + app.globalData.usermessage.tenantId,
            method: 'GET',
            header: {
